...
const TaskEmpleado: React.FC = () => {
  ...
  const [completedDescriptions, setCompletedDescriptions] = useState<boolean[]>(
    []
  );
    ...
    const fetchTaskAndEvidence = async () => {
      try {
        ...
        if (evidences.length > 0) {
          ...
          if (evidences[0].completedDescriptions) {
            setCompletedDescriptions(
              JSON.parse(evidences[0].completedDescriptions)
            );
          } else {
            setCompletedDescriptions(
              new Array(taskData.descripcion.length).fill(false)
            );
          }
        } else {
          setCompletedDescriptions(
  ...
  const handleDeletePhoto = async (
    ...
    try {
      ...
      if (photoId !== undefined) {
        ...
        updatedUploadedPhotos.splice(
          updatedUploadedPhotos.findIndex((p) => p.id === photoId),
          1
        );
    ...
    const handleGuardarYFinalizar = async () => {
        ...
        if (completedDescriptions.includes(false)) {
            Swal.fire({
              title: "Error",
              text: "No has completado todas tus tareas",
              icon: "error",
              confirmButtonText: "Aceptar",
            });
            return;
        ...
        try {
          ...
          if (evidences.length > 0) {
            const updatedEvidence: Evidence = {
              ...evidences[0],
              comentarios: comment,
              completedDescriptions: JSON.stringify(completedDescriptions),
            };
  ...
  const handleDescriptionChange = (index: number) => {
    const newCompletedDescriptions = [...completedDescriptions];
    newCompletedDescriptions[index] = !newCompletedDescriptions[index];
    setCompletedDescriptions(newCompletedDescriptions);
  };
              ...
              <Container className="flex-grow-1 viewPages-fade-in">
                ...
                <Row>
                        ...
                        <li key={index} className="m-2 text-justify">
                          <div className="form-check">
                            <input
                              ...
                              checked={completedDescriptions[index]}
                  ...
                  <Col md={6}>
                    ...
                    <div className="d-flex flex-wrap p-2">
                      ...
                      <label
                        htmlFor="file-input"
                        className="btn btn-secondary align-self-center mb-2"
                        style={{
                          width: "150px",
                          height: "150px",
                          fontSize: "2rem",
                ...
                <Row className="mt-4">
                  <Col className="d-flex justify-content-center">
                    <Button variant="success" onClick={handleGuardarYFinalizar}>
                      Guardar
                    </Button>
                  </Col>
                </Row>
